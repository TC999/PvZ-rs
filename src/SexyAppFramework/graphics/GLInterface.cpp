...
static GLuint gVbo;
static GLint gUfViewProjMtx, gUfTexture, gUfUseTexture, gUfUvBounds, gUfClampUv;
...
V2F vec2 v_uv;
	...
	uniform int u_clampUv;
	void main() {
		if (u_useTexture == 1) {
			vec2 uv = (u_clampUv == 1) ? clamp(v_uv, u_uvBounds.xy, u_uvBounds.zw) : v_uv;
...
static void GfxBindTexture(GLuint tex, const float *uvBounds = kDefaultUvBounds, bool clampUv = true)
{
	...
	glUniform1i(gUfClampUv, clampUv ? 1 : 0);
...
int GLInterface::Init(bool IsWindowed)
{
	...
	if (!inited)
	{
		...
		gUfClampUv     = glGetUniformLocation(gProgram, "u_clampUv");
